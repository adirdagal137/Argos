...
const zoom_routes_1 = __importDefault(require("./routes/zoom.routes"));
const legacy_history_data_1 = require("./legacy_history_data");
const app = (0, express_1.default)();
app.use((0, cors_1.default)());
app.use(express_1.default.json());
const RUNTIME_DIR = 'c:/Users/Widox/Desktop/ARGOS/ARGOS_RUNTIME';
const TOOLS_DIR = path_1.default.join(RUNTIME_DIR, 'tools');
...
const BITACORA_LEGACY_DIR = path_1.default.join(BITACORA_DIR, 'legacy');
const CUBIERTA_DIR = path_1.default.join(RUNTIME_DIR, 'cubierta');
const CUBIERTA_LEGACY_DIR = path_1.default.join(CUBIERTA_DIR, 'legacy');
const ARGOS_GLOBAL_LOG_PATH = path_1.default.join(BITACORA_DIR, 'log.md');
const ARGOS_GLOBAL_SHADOW_PATH = path_1.default.join(BITACORA_DIR, 'shadowlog.md');
const ARGOS_GLOBAL_GLITCH_PATH = path_1.default.join(BITACORA_DIR, 'glitches.md');
const ARGOS_GLOBAL_HANDOFF_LOG_PATH = path_1.default.join(BITACORA_DIR, 'handoffs.md');
const ARGOS_LEGACY_LOG_PATH = path_1.default.join(BITACORA_LEGACY_DIR, 'ARGOS_GLOBAL_LOG.md');
const ARGOS_LEGACY_SHADOW_PATH = path_1.default.join(BITACORA_LEGACY_DIR, 'ARGOS_GLOBAL_SHADOW_LOG.md');
const ARGOS_LEGACY_GLITCH_PATH = path_1.default.join(BITACORA_LEGACY_DIR, 'ARGOS_GLOBAL_GLITCH_LOG.md');
const ARGOS_LEGACY_HANDOFF_LOG_PATH = path_1.default.join(BITACORA_LEGACY_DIR, 'ARGOS_GLOBAL_HANDOFF_LOG.md');
...
const CAPTAIN_FEED_PATH = path_1.default.join(CUBIERTA_DIR, 'feed.jsonl');
const CAPTAIN_FEED_LEGACY_PATH = path_1.default.join(CUBIERTA_LEGACY_DIR, 'captain_feed.jsonl');
const CAPTAIN_FEED_MARKDOWN_PATH = path_1.default.join(CUBIERTA_DIR, 'feed.md');
const CUBIERTA_STATE_PATH = path_1.default.join(CUBIERTA_DIR, 'state.json');
const CUBIERTA_INBOX_SUMMARY_PATH = path_1.default.join(CUBIERTA_DIR, 'inbox_summary.md');
...
const ANTIGRAVITY_PROTO_PACKET_PATH = path_1.default.join(RUNTIME_DIR, 'work_packets', 'inbox', ANTIGRAVITY_PROTO_PACKET_NAME);
const PENDING_ACTIONS_PATH = path_1.default.join(RUNTIME_DIR, 'locks', 'pending_actions.jsonl');
const TRANSCRIPTS_DIR = path_1.default.join(RUNTIME_DIR, 'bitacora', 'legacy', 'transcripts');
const LIVE_DIR = path_1.default.join(RUNTIME_DIR, 'live');
const LIVE_SCHEMA_PATH = path_1.default.join(LIVE_DIR, '_schema.json');
const LEGACY_RUNTIME_DIR = path_1.default.join(RUNTIME_DIR, 'legacy');
const LIVE_DEPRECATED_DIR = path_1.default.join(LEGACY_RUNTIME_DIR, 'live_deprecated_2026-04-18');
const DESKTOP_SOURCES_CONFIG_PATH = path_1.default.join(RUNTIME_DIR, 'state', 'desktop_sources.json');
const DESKTOP_INGEST_STATE_PATH = path_1.default.join(RUNTIME_DIR, 'state', 'desktop_ingest.state.json');
const EXTERNAL_TRANSCRIPTS_DIR = path_1.default.join(TRANSCRIPTS_DIR, 'external');
...
function parseTimestampLabel(rawValue) {
...
function nextFeedMessageId() {
    return `FEED-${Date.now()}-${Math.floor(Math.random() * 1000000)}`;
}
function resolveFeedRecordId(record) {
...
function readCaptainFeedLines(feedPath) {
    if (!fs_1.default.existsSync(feedPath))
        return [];
    const data = fs_1.default.readFileSync(feedPath, 'utf-8').replace(/^\uFEFF/, '');
    return data.split(/\r?\n/).map((line) => {
...
function getCaptainFeedReadPaths() {
...
function readCaptainFeedRecordsFromPaths(paths) {
    const records = [];
    paths.forEach((feedPath) => {
        ...
        readCaptainFeedLines(feedPath).forEach((line) => {
            if (line.parsed)
                records.push(ensureCaptainFeedRecordId(line.parsed));
        });
    });
    return records;
}
function rebuildCubiertaFeedMarkdown() {
    try {
        ...
        fs_1.default.writeFileSync(CAPTAIN_FEED_MARKDOWN_PATH, content, 'utf-8');
    }
    catch (error) {
        console.warn('[CUBIERTA] No se pudo reconstruir feed.md:', error);
    }
}
function appendCaptainFeedRecord(record) {
...
function postToCrewFeed(sender, summary, details = '', kind = 'crew_update', tokens = 0, refId = '') {
    ...
    try {
        ...
        if (numericTokens > 0) {
            const tokensPath = path_1.default.join(RUNTIME_DIR, 'events', 'argos.tokens.jsonl');
...
function appendToHandoffLog(agent, packetId, handoff, timestampIso) {
    try {
        ensureDirSync(path_1.default.dirname(ARGOS_GLOBAL_HANDOFF_LOG_PATH));
...
function readHandoffEntriesForPacket(packetId) {
    const content = [ARGOS_GLOBAL_HANDOFF_LOG_PATH, ARGOS_LEGACY_HANDOFF_LOG_PATH]
        .filter((handoffPath, index, all) => all.indexOf(handoffPath) === index && fs_1.default.existsSync(handoffPath))
        .map((handoffPath) => fs_1.default.readFileSync(handoffPath, 'utf-8'))
        .join('\n---\n');
    if (content === '')
        return [];
...
function transcriptFilePath(agent) {
    if (!fs_1.default.existsSync(TRANSCRIPTS_DIR))
        fs_1.default.mkdirSync(TRANSCRIPTS_DIR, { recursive: true });
    const date = transcriptDateStr();
    const safeName = (agent || 'unknown').replace(/[^a-zA-Z0-9_-]/g, '_');
    return path_1.default.join(TRANSCRIPTS_DIR, `${date}_${safeName}.md`);
}
/**
...
function appendToTranscript(agent, role, content, packetId = '') {
    try {
        ...
        fs_1.default.appendFileSync(filePath, block, 'utf-8');
        const date = transcriptDateStr();
        const safeName = (agent || 'unknown').replace(/[^a-zA-Z0-9_-]/g, '_');
        return `bitacora/legacy/transcripts/${date}_${safeName}.md${packetId ? `#${packetId}` : ''}`;
    }
    catch (e) {
...
function readTranscriptBlock(agent, date, packetId) {
    try {
        ...
        const filePath = path_1.default.join(TRANSCRIPTS_DIR, `${date}_${safeName}.md`);
...
function readArgosState() {
    try {
        if (!fs_1.default.existsSync(ARGOS_STATE_PATH))
            return { open_packets: [] };
        return JSON.parse(fs_1.default.readFileSync(ARGOS_STATE_PATH, 'utf-8'));
    }
    catch (e) {
        return { open_packets: [] };
    }
}
function writeArgosState(state) {
    try {
        state.updated_at = new Date().toISOString();
        const serialized = JSON.stringify(state, null, 4);
        ensureDirSync(path_1.default.dirname(ARGOS_STATE_PATH));
        ensureDirSync(CUBIERTA_DIR);
        fs_1.default.writeFileSync(ARGOS_STATE_PATH, serialized, 'utf-8');
        fs_1.default.writeFileSync(CUBIERTA_STATE_PATH, serialized, 'utf-8');
    }
    catch (e) {
        console.error('[STATE] Error guardando estado de Argos', e);
    }
}
function writeCubiertaInboxSummary() {
    try {
        ensureDirSync(CUBIERTA_DIR);
        ...
        if (!fs_1.default.existsSync(inboxDir)) {
            ...
            const files = fs_1.default.readdirSync(inboxDir).filter((file) => file.endsWith('.md')).sort();
            if (files.length === 0) {
                lines.push('Sin paquetes en inbox.');
            }
            else {
                files.forEach((file) => {
                    const content = readTextFileSafe(path_1.default.join(inboxDir, file));
        ...
        fs_1.default.writeFileSync(CUBIERTA_INBOX_SUMMARY_PATH, `${lines.join('\n')}\n`, 'utf-8');
    }
    catch (error) {
...
function ensureRuntimeCanonicalStorage() {
    ...
    markdownDefaults.forEach((item) => {
        if (!fs_1.default.existsSync(item.path))
            fs_1.default.writeFileSync(item.path, item.header, 'utf-8');
    });
    if (!fs_1.default.existsSync(CAPTAIN_FEED_PATH))
        fs_1.default.writeFileSync(CAPTAIN_FEED_PATH, '', 'utf-8');
    rebuildCubiertaFeedMarkdown();
    writeCubiertaInboxSummary();
}
function canaryDateStamp(now = new Date()) {
...
function canaryTimeStamp(now = new Date()) {
...
function canaryLabel(now = new Date()) {
...
function uniqueDestinationPath(destinationPath) {
    ...
    const ext = path_1.default.extname(destinationPath);
    const base = path_1.default.basename(destinationPath, ext);
    let idx = 1;
    while (true) {
        const candidate = path_1.default.join(dir, `${base}__dup${idx}${ext}`);
...
function moveFileWithFallback(sourcePath, destinationPath) {
    ...
    try {
        fs_1.default.renameSync(sourcePath, finalDestination);
    }
    catch {
        fs_1.default.copyFileSync(sourcePath, finalDestination);
        fs_1.default.unlinkSync(sourcePath);
    }
}
function writeUtf8FileSafe(filePath, content, dryRun, report) {
    if (dryRun) {
...
function moveIfExists(sourcePath, destinationPath, dryRun, report) {
...
function compactDoneStatesIntoArchive(dryRun, report) {
    const activeState = readArgosState();
    const activePacketStates = activeState.packet_states || {};
    const doneEntries = Object.entries(activePacketStates).filter(([, value]) => normaliseText(value).toLowerCase().includes(':done'));
    if (doneEntries.length === 0)
        return 0;
    const stateArchive = readJsonFile(STATE_ARCHIVE_PATH, {
    ...
    for (const [packetId, packetState] of doneEntries) {
        ...
        if (existing === packetState)
            continue;
        let suffix = 1;
        while (stateArchive.packet_states[`${packetId}__dup${suffix}`])
            suffix += 1;
    ...
    fs_1.default.writeFileSync(STATE_ARCHIVE_PATH, JSON.stringify(stateArchive, null, 4), 'utf-8');
    writeArgosState(activeState);
    return doneEntries.length;
}
function resetLiveSessionFiles(dryRun, report) {
    const now = new Date();
    const logHeader = [
        '# ARGOS GLOBAL LOG',
        'Registro operativo compartido por la tripulacion.',
    ...
    const transcriptsLegacyDir = path_1.default.join(RUNTIME_DIR, 'events', 'transcripts');
    if (dryRun) {
...
function archiveDonePacketsToSession(sessionDir, dryRun, report) {
    const doneDir = path_1.default.join(RUNTIME_DIR, 'work_packets', 'done');
    if (!fs_1.default.existsSync(doneDir))
        return 0;
    const doneFiles = fs_1.default.readdirSync(doneDir).filter((name) => name.endsWith('.md'));
    let moved = 0;
    doneFiles.forEach((fileName) => {
        const sourcePath = path_1.default.join(doneDir, fileName);
        const destinationPath = path_1.default.join(sessionDir, 'work_packets', 'done', fileName);
...
function archiveTranscriptLanesToSession(sessionDir, dryRun, report) {
    let moved = 0;
    const transcriptFiles = fs_1.default.existsSync(TRANSCRIPTS_DIR)
        ? fs_1.default.readdirSync(TRANSCRIPTS_DIR).filter((name) => name.endsWith('.md') && name.toLowerCase() !== 'readme.md')
        : [];
    transcriptFiles.forEach((fileName) => {
        const sourcePath = path_1.default.join(TRANSCRIPTS_DIR, fileName);
        const destinationPath = path_1.default.join(sessionDir, 'transcripts', 'live', fileName);
        const movedNow = moveIfExists(sourcePath, destinationPath, dryRun, report);
        if (movedNow)
            moved += 1;
    });
    const legacyTranscriptDir = path_1.default.join(RUNTIME_DIR, 'events', 'transcripts');
    const legacyFiles = fs_1.default.existsSync(legacyTranscriptDir)
        ? fs_1.default.readdirSync(legacyTranscriptDir).filter((name) => name.endsWith('.md'))
        : [];
    legacyFiles.forEach((fileName) => {
        const sourcePath = path_1.default.join(legacyTranscriptDir, fileName);
        const destinationPath = path_1.default.join(sessionDir, 'transcripts', 'legacy_events', fileName);
        const movedNow = moveIfExists(sourcePath, destinationPath, dryRun, report);
        if (movedNow)
            moved += 1;
    });
...
function reorganizeLegacyArtifacts(dryRun, report) {
    const legacyMappings = [];
    const eventsDir = path_1.default.join(RUNTIME_DIR, 'events');
    if (fs_1.default.existsSync(eventsDir)) {
        fs_1.default.readdirSync(eventsDir)
            .filter((name) => /argos\.events\.jsonl\.bak/i.test(name))
            .forEach((name) => {
            legacyMappings.push({
                source: path_1.default.join(eventsDir, name),
                destination: path_1.default.join(LEGACY_ARCHIVE_ROOT, 'events_backups', name)
            });
        });
    }
    legacyMappings.push({
        source: path_1.default.join(RUNTIME_DIR, 'ARGOS_GLOBAL_LOG.md.corrupted.bak'),
        destination: path_1.default.join(LEGACY_ARCHIVE_ROOT, 'file_backups', 'ARGOS_GLOBAL_LOG.md.corrupted.bak')
    }, {
        source: path_1.default.join(RUNTIME_DIR, 'events', 'logs', 'ARGOS_GLOBAL_LOG_archive_20260416.md'),
        destination: path_1.default.join(LEGACY_ARCHIVE_ROOT, 'legacy_global_log_archives', 'ARGOS_GLOBAL_LOG_archive_20260416.md')
    });
    legacyMappings.forEach((mapping) => {
        moveIfExists(mapping.source, mapping.destination, dryRun, report);
    });
}
function runSessionArchive(request) {
    const now = new Date();
    ...
    const sessionDir = path_1.default.join(SESSION_ARCHIVE_ROOT, dateStamp, sessionStamp);
    const report = {
    ...
    const liveFiles = [
        { source: ARGOS_GLOBAL_LOG_PATH, destination: path_1.default.join(sessionDir, 'logs', 'ARGOS_GLOBAL_LOG.md') },
        { source: ARGOS_GLOBAL_SHADOW_PATH, destination: path_1.default.join(sessionDir, 'shadow', 'ARGOS_GLOBAL_SHADOW_LOG.md') },
        { source: ARGOS_GLOBAL_GLITCH_PATH, destination: path_1.default.join(sessionDir, 'glitch', 'ARGOS_GLOBAL_GLITCH_LOG.md') },
        { source: ARGOS_EVENTS_PATH, destination: path_1.default.join(sessionDir, 'events', 'argos.events.jsonl') },
        { source: ARGOS_GLITCHES_PATH, destination: path_1.default.join(sessionDir, 'events', 'argos.glitches.jsonl') },
        { source: ARGOS_TOKENS_PATH, destination: path_1.default.join(sessionDir, 'events', 'argos.tokens.jsonl') },
        { source: CAPTAIN_FEED_PATH, destination: path_1.default.join(sessionDir, 'chat', 'captain_feed.jsonl') }
    ];
    ...
    const manifestPath = path_1.default.join(sessionDir, 'session.archive.manifest.json');
    writeUtf8FileSafe(manifestPath, JSON.stringify({
...
function resolveEstimatedTokens(explicitTokens, ...parts) {
...
function normaliseTokenScope(value) {
...
function inferTokenScope(record) {
    if (record.scope)
        return normaliseTokenScope(record.scope);
...
function looksLikeWorkPacketId(value) {
    const v = normaliseText(value);
    if (!v)
        return false;
    return /^((?:ARG|ARGOS)-[A-Z0-9]+(?:-[A-Z0-9]+)*)$/i.test(v);
}
function hasWorkTokenForPacket(tokensPath, agent, refId) {
    try {
        ...
        for (let i = lines.length - 1; i >= 0 && i >= lines.length - 2000; i -= 1) {
            try {
                ...
                if (normaliseText(parsed.refId) !== needleRefId)
                    continue;
                const parsedAgent = normalizeAgentName(normaliseText(parsed.agent)) || normaliseText(parsed.agent);
                if (parsedAgent !== canonicalAgent)
                    continue;
                if (inferTokenScope(parsed) === 'work')
                    return true;
            }
            catch {
                // ignore malformed
...
function backfillWorkTokensFromReportLedger() {
    try {
        const tokensPath = path_1.default.join(RUNTIME_DIR, 'events', 'argos.tokens.jsonl');
        if (!fs_1.default.existsSync(tokensPath))
            return;
        ...
        readTextFileSafe(tokensPath)
            ...
            try {
                ...
                if (scope === 'report' && looksLikeWorkPacketId(rid))
                    reportCandidates.push(rec);
            }
            catch {
                // ignore malformed
            }
...
function backfillWorkTokensFromFeed() {
    try {
        const tokensPath = path_1.default.join(RUNTIME_DIR, 'events', 'argos.tokens.jsonl');
        const feedPaths = getCaptainFeedReadPaths().filter((feedPath) => fs_1.default.existsSync(feedPath));
        ...
        if (fs_1.default.existsSync(tokensPath)) {
            readTextFileSafe(tokensPath)
                ...
                try {
                    const parsed = JSON.parse(line);
                    const key = `${normaliseText(parsed.timestamp)}|${normaliseText(parsed.agent)}|${String(parsed.tokens ?? '')}|${normaliseText(parsed.refId)}|${normaliseText(parsed.channel)}`;
        ...
        feedPaths.forEach((feedPath) => {
            readTextFileSafe(feedPath)
                ...
                .forEach((line) => {
                try {
                    const feedRecord = JSON.parse(line);
...
function isAgentRestricted(actor) {
    const state = readArgosState();
...
function interceptAction(req, res, actionType, executeFn) {
    ...
    if (isAgentRestricted(actor)) {
        ...
        if (!fs_1.default.existsSync(path_1.default.dirname(PENDING_ACTIONS_PATH))) {
            fs_1.default.mkdirSync(path_1.default.dirname(PENDING_ACTIONS_PATH), { recursive: true });
        }
...
function readTextFile(filePath, fallback = '') {
    return fs_1.default.existsSync(filePath) ? fs_1.default.readFileSync(filePath, 'utf-8') : fallback;
}
function readTextFilesJoined(filePaths, fallback = '') {
    const content = filePaths
        .filter((filePath, index, all) => all.indexOf(filePath) === index && fs_1.default.existsSync(filePath))
...
function resolveRemoteAgentTokenKey(rawAgent) {
    const normalized = normaliseText(rawAgent).toLowerCase();
    if (normalized.includes('claude') || normalized.includes('orfeo'))
        return 'Claude';
...
function buildDefaultAgentTokens() {
    return {
        Claude: `arg_clau_${(0, crypto_1.randomBytes)(24).toString('hex')}`,
        ChatGPT: `arg_chat_${(0, crypto_1.randomBytes)(24).toString('hex')}`,
        Gemini: `arg_gemi_${(0, crypto_1.randomBytes)(24).toString('hex')}`
    };
}
function ensureAgentTokensFile() {
    ensureDirSync(SECRETS_DIR);
    const current = readJsonFile(AGENT_TOKENS_PATH, {});
    const defaults = buildDefaultAgentTokens();
    const next = { ...current };
    let changed = false;
    Object.keys(defaults).forEach((agent) => {
    ...
    if (!fs_1.default.existsSync(AGENT_TOKENS_PATH) || changed) {
...
function buildRemoteClosureKey(agent, packetId, timestampIso) {
    return `${normaliseText(agent).toLowerCase()}|${normaliseText(packetId)}|${normaliseText(timestampIso)}`;
}
function hasRemoteClosureRecord(key) {
    ...
    for (const line of rows) {
        try {
            const parsed = JSON.parse(line);
            if (normaliseText(parsed.key) === key)
                return true;
        }
        catch {
...
function appendRemoteClosureRecord(record) {
    appendJsonlRecord(ARGOS_REMOTE_CLOSURES_PATH, record);
}
function canonicalLiveAgentName(agentId) {
    if (agentId === 'claude')
        return 'Claude';
    if (agentId === 'codex')
        return 'Codex';
...
function normalizeLiveAgentId(raw) {
    ...
    if (value === 'claude' || value === 'orfeo')
        return 'claude';
    if (value === 'codex' || value === 'chatgpt')
        return 'codex';
    if (value === 'gemini' || value === 'antigravity' || value === 'ag')
        return 'gemini';
    if (value === 'openclaw' || value === 'qwen')
        return 'openclaw';
    return null;
}
function liveAgentToIaStatusKey(agentId) {
    if (agentId === 'claude')
        return 'Claude';
    if (agentId === 'codex')
        return 'Codex';
...
function normalizeLiveStatus(raw) {
    ...
    if (value === 'blocked')
        return 'blocked';
    if (value === 'waiting_captain')
        return 'waiting_captain';
    return 'idle';
}
function defaultLiveState(agentId) {
    return {
...
function sanitizeLiveState(agentId, input) {
...
function mapIaStatusToLiveStatus(status) {
    if (status === 'active')
        return 'working';
...
function mapLiveStatusToIaStatus(status) {
    if (status === 'idle')
        return 'standby';
    return 'active';
}
function isLiveStateStale(updatedAt, staleHours = 24) {
    const ts = Date.parse(updatedAt);
    if (Number.isNaN(ts))
        return true;
    return (Date.now() - ts) > (staleHours * 60 * 60 * 1000);
}
function liveSchemaTemplate() {
    return {
...
function archiveLiveLayerToLegacyIfNeeded() {
    const markerPath = path_1.default.join(LIVE_DEPRECATED_DIR, '_deprecation.json');
    if (!fs_1.default.existsSync(LIVE_DIR)) {
    ...
    ensureDirSync(LIVE_DEPRECATED_DIR);
    const entries = fs_1.default.readdirSync(LIVE_DIR, { withFileTypes: true });
    entries.forEach((entry) => {
        const sourcePath = path_1.default.join(LIVE_DIR, entry.name);
        const destinationPath = path_1.default.join(LIVE_DEPRECATED_DIR, entry.name);
    ...
    try {
        fs_1.default.rmSync(LIVE_DIR, { recursive: true, force: true });
    }
    catch {
...
function ensureLiveLayerBootstrap() {
...
function readLiveState(agentId) {
    const state = readArgosState();
    const ia = readIaStatus(state);
    const iaKey = liveAgentToIaStatusKey(agentId);
    const row = ia[iaKey];
    const updatedAt = normaliseText(row.last_seen) || row.since || nowIso();
    const handoffId = normalizeLiveAgentId(row.handoff_to);
    const packetId = normaliseText(row.task) || null;
    const output = normaliseText(row.last_output) || row.task_subject || 'Estado sincronizado desde ia_status.';
    return {
        agent: canonicalLiveAgentName(agentId),
        ...
        last_output: output,
        open_question: null,
        next_step: normaliseText(row.next_step) || null,
        handoff_to: handoffId ? canonicalLiveAgentName(handoffId) : null
    };
}
function writeLiveState(agentId, payload) {
...
function parseDepositTimestampIso(raw) {
...
function parseDepositSections(body) {
    const sections = {};
...
function parseDepositStatePayload(rawState) {
    const parsed = {};
    rawState.split(/\r?\n/).forEach((line) => {
...
function appendMarkdownEntry(filePath, fallbackHeader, entry) {
    ensureDirSync(path_1.default.dirname(filePath));
    const existing = fs_1.default.existsSync(filePath) ? fs_1.default.readFileSync(filePath, 'utf-8') : fallbackHeader;
    const base = existing.endsWith('\n') ? existing : `${existing}\n`;
    fs_1.default.writeFileSync(filePath, `${base}${entry}`, 'utf-8');
}
...
function updateIaStatusFromDeposit(deposit) {
    ...
    if (deposit.statePayload.next_step)
        state.next_step = deposit.statePayload.next_step;
    writeArgosState(state);
}
function integrateClosure(deposit, options) {
    ...
    if (!isEmptyGlitchSection(glitchText)) {
        const glitchEntry = `\n---\n` +
            ...
            `**PACKET:** ${packetRef || 'N/A'}\n` +
            `${glitchText}\n`;
        appendMarkdownEntry(ARGOS_GLOBAL_GLITCH_PATH, '# ARGOS GLOBAL GLITCH LOG\nRegistro estructural de fallas sistemicas y fricciones tecnicas.\n', glitchEntry);
...
function markStaleAgentsFromDeposits(staleMinutes = 60) {
    const state = readArgosState();
    const ia = readIaStatus(state);
    let changes = 0;
    Object.keys(ia).forEach((agentKey) => {
        const row = ia[agentKey];
        const seenRaw = normaliseText(row.last_seen) || row.since;
        const seenTs = Date.parse(seenRaw);
        if (Number.isNaN(seenTs))
            return;
        const isStaleNow = (Date.now() - seenTs) > staleMinutes * 60 * 1000;
        const wasStale = Boolean(row.stale);
...
function defaultDesktopSourcesConfig() {
    return {
        version: 1,
        updated_at: nowIso(),
...
function loadDesktopSourcesConfig() {
    const fallback = defaultDesktopSourcesConfig();
    if (!fs_1.default.existsSync(DESKTOP_SOURCES_CONFIG_PATH)) {
        writeJsonFileSafe(DESKTOP_SOURCES_CONFIG_PATH, fallback);
        return fallback;
    }
    const cfg = readJsonFile(DESKTOP_SOURCES_CONFIG_PATH, fallback);
...
function saveDesktopSourcesConfig(config) {
    writeJsonFileSafe(DESKTOP_SOURCES_CONFIG_PATH, {
        ...config,
        updated_at: nowIso()
    });
}
function emptyDesktopIngestState() {
    return {
        version: 1,
        updated_at: nowIso(),
        files: {},
...
function loadDesktopIngestState() {
    const fallback = emptyDesktopIngestState();
    if (!fs_1.default.existsSync(DESKTOP_INGEST_STATE_PATH)) {
        writeJsonFileSafe(DESKTOP_INGEST_STATE_PATH, fallback);
        return fallback;
    }
    const loaded = readJsonFile(DESKTOP_INGEST_STATE_PATH, fallback);
...
function saveDesktopIngestState(state) {
...
function listFilesRecursive(rootPath, limit = 30000) {
    ...
    while (stack.length > 0 && out.length < limit) {
        ...
        let items = [];
        try {
            items = fs_1.default.readdirSync(current, { withFileTypes: true });
        }
        catch {
            continue;
        }
        for (const item of items) {
            const full = path_1.default.join(current, item.name);
            if (item.isDirectory()) {
                stack.push(full);
                continue;
            }
...
function filesMatchingGlobs(rootPath, globs) {
    if (!fs_1.default.existsSync(rootPath))
        return [];
    ...
    return all.filter((abs) => {
        const rel = path_1.default.relative(rootPath, abs).replace(/\\/g, '/');
        return regexes.some((rx) => rx.test(rel));
    });
}
function extractTimestampFromUnknown(input, fallbackIso) {
...
function extractTokenTotalFromObject(obj) {
    const directFields = [
        'tokens',
        'total_tokens',
        'totalTokenCount',
...
function parseTokenRecordsFromFile(filePath, source) {
    const content = readTextFileSafe(filePath);
    if (content.trim() === '')
        return [];
    const fallbackIso = new Date(fs_1.default.statSync(filePath).mtimeMs).toISOString();
        ...
        if (line.startsWith('{') && line.endsWith('}')) {
            try {
                ...
                if (tokens > 0) {
                    ...
                    const ref = normaliseText(String(parsed.ref ?? parsed.action ?? parsed.type ?? parsed.model ?? path_1.default.basename(filePath))).slice(0, 180) || path_1.default.basename(filePath);
            ...
            if (match) {
                ...
                if (tokenVal > 0) {
                    out.push({
                        ...
                        ref: `${source.id}:${path_1.default.basename(filePath)}`.slice(0, 180),
                        refId: ''
                    });
                }
                break;
        ...
        try {
            ...
            arr.forEach((entry) => {
                ...
                out.push({
                    ...
                    tokens,
                    ref: normaliseText(String(rec.ref ?? rec.action ?? rec.type ?? path_1.default.basename(filePath))).slice(0, 180) || path_1.default.basename(filePath),
...
function sanitiseRelativePath(rawPath) {
    return rawPath
        .replace(/\\/g, '/')
        .split('/')
...
function markImportedKey(list, key) {
...
function runDesktopImport(mode, actor = 'OpenClaw') {
    ...
    enabledSources.forEach((source) => {
        if (!fs_1.default.existsSync(source.rootPath)) {
        ...
        if (mode === 'tokens' || mode === 'all') {
            ...
            tokenFiles.forEach((filePath) => {
                try {
                    ...
                    if (prev && prev.mtimeMs === stat.mtimeMs && prev.size === stat.size && prev.hash === hash)
                        return;
                    ...
                    rows.forEach((row, idx) => {
                        ...
                        if (!markImportedKey(state.importedTokenKeys, key))
                            return;
        ...
        if (mode === 'transcripts' || mode === 'all') {
            ...
            transcriptFiles.forEach((filePath) => {
                try {
                    ...
                    const raw = readTextFileSafe(filePath);
                    const hash = simpleHash(raw);
                    const rel = sanitiseRelativePath(path_1.default.relative(source.rootPath, filePath));
                    const destination = path_1.default.join(EXTERNAL_TRANSCRIPTS_DIR, source.id, rel);
                    const stateKey = `transcript:${filePath}`;
                    const prev = state.files[stateKey];
                    if (prev && prev.mtimeMs === stat.mtimeMs && prev.size === stat.size && prev.hash === hash)
                        return;
                    ensureDirSync(path_1.default.dirname(destination));
                    fs_1.default.writeFileSync(destination, raw, 'utf-8');
...
function findArchivedFiles(filename, dirPath = SESSION_ARCHIVE_ROOT) {
    const results = [];
    try {
        if (!fs_1.default.existsSync(dirPath))
            return results;
...
function appendJsonlRecord(filePath, record) {
    ...
    const dedupFingerprint = (dedupKind !== '' || dedupSummary !== '')
...
function recordTokensInternal(agent, tokens, ref, refId = '', scope = 'work') {
    if (tokens <= 0)
        return;
    const record = {
        timestamp: new Date().toISOString(),
...
function composeShadowNote(params) {
    ...
    if (discarded !== '')
        lines.push(`Descartado: ${discarded}`);
    if (ignored !== '')
        lines.push(`Ignorado: ${ignored}`);
    if (suppressed !== '')
        lines.push(`Suprimido: ${suppressed}`);
    if (errors !== '')
        lines.push(`Aprendizajes: ${errors}`);
    if (lines.length > 0)
        return lines.join('\n');
    if (details !== '')
...
function normalisePacketId(raw) {
    return normaliseText(raw).toUpperCase();
}
function buildTriLogViolationKey(packetId, missingLanes) {
    return `${normalisePacketId(packetId)}|${missingLanes.map((lane) => normaliseText(lane).toUpperCase()).sort().join(',')}`;
}
function hasOpenTriLogViolation(packetId, missingLanes) {
    ...
    return readTextFileSafe(ARGOS_GLITCHES_PATH)
        ...
        try {
            const parsed = JSON.parse(line);
            if (normaliseText(parsed.type) !== 'trilog_violation')
                return false;
            if (normaliseText(parsed.status).toLowerCase() !== 'open')
                return false;
...
function hasPacketIdInMarkdownLane(filePath, packetId) {
    ...
    for (const tagRegex of tagRegexes) {
        ...
        while (match) {
            if (normalisePacketId(match[1]) === needle)
                return true;
...
function hasPacketIdInAnyMarkdownLane(filePaths, packetId) {
    return filePaths.some((filePath) => hasPacketIdInMarkdownLane(filePath, packetId));
}
function hasPacketIdInEventsLane(packetId) {
    const needle = normalisePacketId(packetId);
    if (needle === '')
        return false;
...
function hasPacketIdInTranscriptLane(packetId) {
    const needle = normalisePacketId(packetId);
    if (needle === '')
        return false;
    const transcriptDir = path_1.default.join(RUNTIME_DIR, 'transcripts');
    if (!fs_1.default.existsSync(transcriptDir))
        return false;
    const files = fs_1.default.readdirSync(transcriptDir).filter(f => f.endsWith('.md'));
    for (const file of files) {
        const content = fs_1.default.readFileSync(path_1.default.join(transcriptDir, file), 'utf-8');
...
function getTriLogMissingLanes(packetId) {
    const id = normaliseText(packetId);
    if (id === '')
        return ['LOG', 'SHADOW', 'TRANSCRIPT'];
...
2. Registrar cierre en bitacora/log.md (LOG lane).
3. Registrar evento estructurado en events/argos.events.jsonl (EVENTS lane).
4. Registrar sombra en bitacora/shadowlog.md (SHADOW lane).
5. Sincronizar state despues del cierre.
[/WORK_PACKET]
`;
    try {
        fs_1.default.writeFileSync(ANTIGRAVITY_PROTO_PACKET_PATH, content, 'utf-8');
    }
    catch (error) {
        console.warn('[TRILOG-GUARD] No se pudo crear ARG-PROTO-AG-001:', error);
    }
}
function recordTriLogViolation(packetId, subject, owner, missingLanes) {
...
function runPowerShellScript(scriptName, timeoutMs) {
    const scriptPath = path_1.default.join(TOOLS_DIR, scriptName);
    ...
    const result = (0, child_process_1.spawnSync)('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', scriptPath, '-RuntimeDir', RUNTIME_DIR], { encoding: 'utf-8', timeout: timeoutMs, windowsHide: true });
    return {
        ok: result.status === 0,
        code: result.status,
...
function runSyncState() {
    try {
        ...
        const defaultState = {
            ...
            done_packets_summary: { total: 0, tasks: 0, bugs: 0, latest_ids: [] }
        };
        const state = readJsonFile(statePath, defaultState);
        // 2. Escaneo de zonas
        const getPackets = (zone) => {
            const zonePath = path_1.default.join(workPacketsDir, zone);
            if (!fs_1.default.existsSync(zonePath))
                return [];
            return fs_1.default.readdirSync(zonePath)
                .filter(f => f.endsWith('.md') && !f.includes('__RESULTADO'))
                .map(f => {
                const content = fs_1.default.readFileSync(path_1.default.join(zonePath, f), 'utf-8');
...
function runMergeHistory() {
    try {
        const historyDir = path_1.default.join(RUNTIME_DIR, 'views', 'history_export');
        const historyTimelinePath = path_1.default.join(historyDir, 'argos.timeline.jsonl');
        const historyMarkdownPath = path_1.default.join(historyDir, 'ARGOS_CANONICAL_HISTORY.md');
        const historyManifestPath = path_1.default.join(historyDir, 'current_history_manifest.json');
        ...
        legacy_history_data_1.manualDraftShadows.forEach(e => addEntry(e, 'shadow'));
        legacy_history_data_1.legacyGlitches.forEach(e => addEntry(e, 'glitch'));
        legacy_history_data_1.lateRuntimeGlobals.forEach(e => addEntry(e, 'global'));
        legacy_history_data_1.lateRuntimeShadows.forEach(e => addEntry(e, 'shadow'));
        // 2. Leer Logbook actual
        const logbook = readJsonFile(LOGBOOK_SNAPSHOT_PATH, buildEmptyLogbook());
        ...
        const doneDir = path_1.default.join(RUNTIME_DIR, 'work_packets', 'done');
        if (fs_1.default.existsSync(doneDir)) {
            const files = fs_1.default.readdirSync(doneDir).filter(f => f.endsWith('.md'));
            files.forEach(f => {
        ...
        fs_1.default.writeFileSync(historyTimelinePath, timelineJsonl, 'utf-8');
        ...
        fs_1.default.writeFileSync(LOGBOOK_SNAPSHOT_PATH, JSON.stringify(logbook, null, 2), 'utf-8');
...
function getMtimeMs(targetPath) {
    try {
        return fs_1.default.statSync(targetPath).mtimeMs;
    }
    catch (error) {
        return 0;
    }
...
function shouldRefreshLogbookSnapshot() {
    const sourcePaths = [
        ...
        path_1.default.join(RUNTIME_DIR, 'work_packets', 'inbox'),
        path_1.default.join(RUNTIME_DIR, 'work_packets', 'in_progress'),
        path_1.default.join(RUNTIME_DIR, 'work_packets', 'done')
    ];
    const latestSourceMtime = sourcePaths.reduce((max, currentPath) => Math.max(max, getMtimeMs(currentPath)), 0);
    if (!fs_1.default.existsSync(LOGBOOK_SNAPSHOT_PATH))
        return true;
    return getMtimeMs(LOGBOOK_SNAPSHOT_PATH) < latestSourceMtime;
}
function maybeRefreshLogbookSnapshot(force = false) {
    const now = Date.now();
    if (logbookRefreshInFlight)
        return null;
    const minIntervalMs = 5000;
    if (!force && now - lastLogbookRefreshAttemptMs < minIntervalMs)
        return null;
    if (!force && !shouldRefreshLogbookSnapshot())
        return null;
...
function inferSenderName(record) {
...
function extractPacketIdFromArtifacts(artifacts) {
    if (!Array.isArray(artifacts))
        return '';
    const wpPath = artifacts.find((a) => typeof a === 'string' && a.includes('work_packets'));
    if (!wpPath || typeof wpPath !== 'string')
        return '';
    const filename = wpPath.split('/').pop() || '';
    const match = filename.match(/^([A-Z0-9-]+?)(?:__|\.md)/i);
    return match ? match[1] : '';
}
function parseArgosMarkdownStream(filePath, idPrefix) {
    if (!fs_1.default.existsSync(filePath))
        return [];
    const content = fs_1.default.readFileSync(filePath, 'utf-8');
    ...
    blocks.forEach((block, index) => {
        ...
        const explicitPacketId = block.match(/^\s*\*\*WORK\s*PACKET:\*\*\s*([^\n]+)/im)?.[1]?.trim() ||
            ...
            headerId;
        const timestampRaw = block.match(/\[(\d{4}-\d{2}-\d{2}(?:\s+\d{2}:\d{2})?)/)?.[1] ||
        ...
        const detailsRaw = block.match(/\*\*(?:DETALLES|DETAILS)(?:\s+\d+)?\s*:\*\*([\s\S]*?)(?:\n\*\*|\n---|$)/i)?.[1] ||
            block.match(/\*\*ACCIONES(?:\s+EJECUTADAS)?\s*:\*\*([\s\S]*?)(?:\n\*\*|\n---|$)/i)?.[1] ||
            block.match(/\*\*SOMBRA\s*:\*\*([\s\S]*?)(?:\n\*\*|\n---|$)/i)?.[1] ||
            block.match(/\*\*NOTA\s*:\*\*([\s\S]*?)(?:\n\*\*|\n---|$)/i)?.[1] ||
            '';
        const nextStep = block.match(/\*\*(?:SIGUIENTE PASO|SIGUIENTE|NEXT STEP|Paso Siguiente)\s*:\*\*\s*([^\n]+)/i)?.[1]?.trim() ||
            '';
...
function parseArgosEventsStream(filePath, idPrefix) {
    if (!fs_1.default.existsSync(filePath))
        return [];
...
function parseCaptainFeedStream(filePath, idPrefix) {
    if (!fs_1.default.existsSync(filePath))
        return [];
    const lines = fs_1.default.readFileSync(filePath, 'utf-8').split('\n').map((l) => l.trim()).filter((l) => l !== '');
    const parsed = [];
...
function buildWorkPacketCache() {
    const cache = new Map();
    const zones = ['inbox', 'in_progress', 'done'];
    for (const zone of zones) {
        const zonePath = path_1.default.join(RUNTIME_DIR, 'work_packets', zone);
        if (!fs_1.default.existsSync(zonePath))
            continue;
        try {
            const files = fs_1.default.readdirSync(zonePath).filter((f) => f.endsWith('.md'));
            for (const file of files) {
                const content = fs_1.default.readFileSync(path_1.default.join(zonePath, file), 'utf-8');
                const idMatch = content.match(/ID:\s*([^\n]+)/i);
                const subjectMatch = content.match(/SUBJECT:\s*([^\n]+)/i);
                const ownerMatch = content.match(/ROLE_REQUESTED:\s*([^\n]+)/i) ||
                    content.match(/OWNER:\s*([^\n]+)/i);
                if (idMatch) {
...
function enrichArgosStreams(snapshot) {
    const clone = JSON.parse(JSON.stringify(snapshot || buildEmptyLogbook()));
    ...
    const loadStreamEntries = (paths, streamType, parser) => {
        const combined = [];
    ...
    const applyEntries = (streamId, entries) => {
        ...
        deduplicated.sort((a, b) => {
            const msA = a.sortMs || Date.parse(a.timestamp || a.timestamp_label || '1970-01-01') || 0;
...
function inferTaskOwner(rawText) {
...
function inferTaskSubject(rawText) {
    const collapsed = rawText.replace(/\r/g, '').replace(/\n+/g, ' ').replace(/\s+/g, ' ').trim();
    if (collapsed === '')
        return 'Conversacion Viva Mapeada';
    const withoutDirectAddress = collapsed.replace(/^(codex|claude|orfeo|antigravity|gemini|capitan|jason|thor)\b[\s,:-]*/i, '').trim();
...
function subjectToPacketSuffix(subject) {
...
function formatRelativeAge(mtimeMs) {
...
function getPacketField(content, fieldName) {
...
function normalizeTaskStatus(status, fallback = 'open') {
    ...
    if (['in_progress', 'in-progress', 'progress', 'active', 'doing', 'review'].includes(raw))
        return 'in_progress';
    if (['open', 'inbox', 'pending', 'todo', 'queued'].includes(raw))
        return 'open';
    return fallback;
}
...
function upsertPacketField(content, fieldName, nextValue) {
...
function replacePacketObjective(content, objective) {
...
function findWorkPacketById(packetId, zones = ['inbox', 'in_progress', 'done']) {
    ...
    for (const zone of zones) {
        ...
        if (!fs_1.default.existsSync(zonePath))
            continue;
        const files = fs_1.default.readdirSync(zonePath).filter((file) => file.endsWith('.md') && !file.includes('__RESULTADO'));
        for (const fileName of files) {
            const filePath = path_1.default.join(zonePath, fileName);
            const content = fs_1.default.readFileSync(filePath, 'utf-8');
            if (!content.includes('[WORK_PACKET]'))
                continue;
            const parsedId = getPacketField(content, 'ID') || fileName.replace(/\.md$/i, '');
            if (normaliseText(parsedId) !== targetId)
                continue;
            return {
                packetId: parsedId,
                zone,
...
function packetExistsInZone(packetId, zone) {
    const targetId = normaliseText(packetId);
    if (targetId === '')
        return false;
    const zonePath = path_1.default.join(RUNTIME_DIR, 'work_packets', zone);
    if (!fs_1.default.existsSync(zonePath))
        return false;
    const files = fs_1.default.readdirSync(zonePath).filter((file) => file.endsWith('.md'));
...
function markWorkPacketDone(packetId, actor, timestampIso) {
    const targetId = normaliseText(packetId);
    if (targetId === '')
        return { moved: false, reason: 'packet_id vacio' };
    ...
    const targetPath = path_1.default.join(doneDir, resolved.fileName);
    fs_1.default.writeFileSync(targetPath, nextContent, 'utf-8');
    if (resolved.filePath !== targetPath && fs_1.default.existsSync(resolved.filePath)) {
        fs_1.default.unlinkSync(resolved.filePath);
    }
    const state = readArgosState();
    if (!state.packet_states)
        state.packet_states = {};
    state.packet_states[targetId] = 'done:done';
    if (Array.isArray(state.open_packets)) {
        state.open_packets = state.open_packets.filter((id) => id !== targetId);
    }
    if (Array.isArray(state.in_progress_packets)) {
        state.in_progress_packets = state.in_progress_packets.filter((id) => id !== targetId);
    }
    writeArgosState(state);
...
function parseLooseTimestampMs(rawValue) {
    const trimmed = normaliseText(rawValue);
    if (trimmed === '')
        return 0;
    const parsedIso = Date.parse(trimmed);
...
function buildSignalHash(rawText) {
...
function extractLolaStressLevel(packetContent) {
...
function loadOpenLolaRiskPackets() {
    ...
    zones.forEach((zone) => {
        ...
        const files = fs_1.default.readdirSync(zonePath).filter((file) => file.endsWith('.md'));
        files.forEach((file) => {
            const fullPath = path_1.default.join(zonePath, file);
            ...
            const isLolaRisk = packetType === 'risk' &&
            ...
            const mtimeMs = fs_1.default.statSync(fullPath).mtimeMs;
            packets.push({
...
function loadTasksFromZone(zone) {
    const zonePath = path_1.default.join(RUNTIME_DIR, 'work_packets', zone);
    if (!fs_1.default.existsSync(zonePath))
        return [];
    const files = fs_1.default.readdirSync(zonePath).filter((file) => file.endsWith('.md') && !file.includes('__RESULTADO'));
    const tasks = files.map((file) => {
        ...
        if (priority === '') {
            ...
            if (priority.includes('p1') || priority.includes('high'))
                priority = 'high';
            else if (priority.includes('p2') || priority.includes('mid'))
                priority = 'mid';
            else
                priority = 'low';
        }
...
function isIaGenericOwner(ownerText) {
...
function isTaskAssignedToAgent(task, agentName) {
...
function loadGlitchesFromJsonl() {
    const glitchesPath = path_1.default.join(RUNTIME_DIR, 'events', 'argos.glitches.jsonl');
    if (!fs_1.default.existsSync(glitchesPath))
        return [];
    try {
        const lines = fs_1.default.readFileSync(glitchesPath, 'utf-8').split('\n').filter(l => l.trim());
        const byIssue = new Map();
        lines.forEach(line => {
            try {
                const g = JSON.parse(line);
                const lanes = Array.isArray(g.missing_lanes)
                    ? g.missing_lanes.map((lane) => normaliseText(lane)).sort().join(',')
                    : '';
                const issueKey = [
...
function loadBugsFromAllZones() {
    ...
    for (const zone of zones) {
        ...
        if (!fs_1.default.existsSync(zonePath))
            continue;
        const files = fs_1.default.readdirSync(zonePath).filter((f) => f.endsWith('.md') && !f.includes('__RESULTADO'));
        for (const file of files) {
            const fullPath = path_1.default.join(zonePath, file);
            const content = fs_1.default.readFileSync(fullPath, 'utf-8');
            if (!content.includes('[WORK_PACKET]'))
...
function readMarkdownTailFromPaths(filePaths, maxLines) {
    ...
    const scanPackets = () => {
        ...
        zones.forEach(zone => {
            ...
            files.forEach(filename => {
                const fullPath = path_1.default.join(zonePath, filename);
                const stat = fs_1.default.statSync(fullPath);
...
app.post('/api/admin/rotate-token', (req, res) => {
    try {
        ...
        const rawAgent = normaliseText(req.query.agent) || normaliseText(req.body?.agent);
        if (rawAgent === '') {
        ...
        if (tokenKey === 'Gemini')
            prefix = 'arg_gemi_';
        tokens[tokenKey] = `${prefix}${(0, crypto_1.randomBytes)(24).toString('hex')}`;
...
app.post('/api/refresh-logbook', (req, res) => {
    ...
    return res.json({
        ...
        code: refreshResult.code,
        stdout: refreshResult.stdout,
        logbook: enrichArgosStreams(readJsonFile(LOGBOOK_SNAPSHOT_PATH, buildEmptyLogbook()))
    });
});
app.post('/api/session/archive', (req, res) => {
    try {
        const { mode = 'end_of_day', actor = 'Codex', reason = 'Rollover de sesion', dryRun = false, archiveDonePackets = true, compactState = true, reorganizeLegacy = true } = req.body || {};
...
app.post('/api/chat', (req, res) => {
    try {
        ...
        if (looksLikeWorkPacketId(packetRefId) && !hasWorkTokenForPacket(recordsPath, canonicalSender, packetRefId)) {
            ...
            const workTokens = Number.isFinite(explicitWork) && explicitWork > 0
...
app.post('/api/chat/edit', (req, res) => {
    try {
        ...
        const feedPath = getCaptainFeedReadPaths().find((candidatePath) => readCaptainFeedLines(candidatePath).some((line) => line.parsed && resolveFeedRecordId(ensureCaptainFeedRecordId(line.parsed)) === messageId));
        if (!feedPath)
...
app.get('/api/logbook', (req, res) => {
    try {
        const snapshot = readJsonFile(LOGBOOK_SNAPSHOT_PATH, buildEmptyLogbook());
...
app.get('/api/tokens', (req, res) => {
    try {
        ...
        const data = fs_1.default.readFileSync(tokensPath, 'utf-8').replace(/^\uFEFF/, '');
        ...
        const scopeFilter = effectiveScope === 'all' ? 'all' : normaliseTokenScope(effectiveScope);
        ...
        const lines = unfilteredLines.filter(line => {
            try {
                ...
                if (refId && record.refId !== refId)
                    return false;
                if (scopeFilter !== 'all' && inferTokenScope(record) !== scopeFilter)
                    return false;
                return true;
            }
            catch (e) {
...
app.post('/api/tokens/record', (req, res) => {
    try {
        ...
        if (resolvedTokens <= 0)
            return res.status(400).json({ error: 'Missing tokens or estimable context' });
...
app.post('/api/trilog', (req, res) => {
    try {
        const { actor, packetId = '', summary, details = '', nextStep = '', errors = '', risks = '', tokens = 0, // legacy fallback => processTokens
...
app.get('/api/transcript', (req, res) => {
    try {
        const agent = normaliseText(req.query.agent) || 'Claude';
        const date = normaliseText(req.query.date) || transcriptDateStr();
        const packetId = normaliseText(req.query.packetId);
        if (packetId) {
            const block = readTranscriptBlock(agent, date, packetId);
            if (!block)
                return res.status(404).json({ error: `No transcript block for ${packetId} in ${date}_${agent}` });
            return res.json({ agent, date, packetId, content: block });
        }
        const safeName = agent.replace(/[^a-zA-Z0-9_-]/g, '_');
        const filePath = path_1.default.join(TRANSCRIPTS_DIR, `${date}_${safeName}.md`);
        if (!fs_1.default.existsSync(filePath))
            return res.status(404).json({ error: `No transcript for ${agent} on ${date}` });
        const content = fs_1.default.readFileSync(filePath, 'utf-8');
...
app.get('/api/transcript/list', (req, res) => {
    try {
        const date = normaliseText(req.query.date) || transcriptDateStr();
        if (!fs_1.default.existsSync(TRANSCRIPTS_DIR))
...
app.post('/api/transcript', (req, res) => {
    try {
        ...
        const tRef = appendToTranscript(normalizeAgentName(agent) || agent, validRole, content, packetId || '');
...
app.get('/api/transcript/packet', (req, res) => {
    try {
        const packetId = normaliseText(req.query.packetId);
        if (!packetId)
            return res.status(400).json({ error: 'packetId requerido' });
...
app.get('/api/transcript/feed', (req, res) => {
    try {
        const date = normaliseText(req.query.date) || transcriptDateStr();
        if (!fs_1.default.existsSync(TRANSCRIPTS_DIR))
        ...
        for (const file of files) {
            ...
            const rawContent = fs_1.default.readFileSync(path_1.default.join(TRANSCRIPTS_DIR, file), 'utf-8');
...
app.get('/api/ia/pending-actions', (_req, res) => {
    if (!fs_1.default.existsSync(PENDING_ACTIONS_PATH))
        return res.json([]);
    const lines = fs_1.default.readFileSync(PENDING_ACTIONS_PATH, 'utf-8').split('\n').filter(l => l.trim() !== '');
...
app.post('/api/ia/approve-action', (req, res) => {
    ...
    fs_1.default.writeFileSync(PENDING_ACTIONS_PATH, nextLines === '' ? '' : nextLines + '\n');
...
app.post('/api/tasks/update', (req, res) => {
    ...
    interceptAction(req, res, 'tasks/update', () => {
        try {
            ...
            if (nextOwner === '')
                return res.status(400).json({ error: 'owner no puede quedar vacio' });
            if (nextObjective === '')
            ...
            const nextPath = path_1.default.join(RUNTIME_DIR, 'work_packets', nextZone);
            if (!fs_1.default.existsSync(nextPath))
                fs_1.default.mkdirSync(nextPath, { recursive: true });
            const targetFilePath = path_1.default.join(nextPath, resolved.fileName);
            fs_1.default.writeFileSync(targetFilePath, nextContent, 'utf-8');
            if (targetFilePath !== resolved.filePath && fs_1.default.existsSync(resolved.filePath)) {
                fs_1.default.unlinkSync(resolved.filePath);
            }
...
app.post('/api/ia/start-task', (req, res) => {
    try {
        ...
        let filename = null;
        for (const zone of zones) {
            const dir = path_1.default.join(RUNTIME_DIR, 'work_packets', zone);
        ...
        const inProgressDir = path_1.default.join(RUNTIME_DIR, 'work_packets', 'in_progress');
        if (!fs_1.default.existsSync(inProgressDir))
            fs_1.default.mkdirSync(inProgressDir, { recursive: true });
        let content = fs_1.default.readFileSync(foundPath, 'utf-8');
        ...
        const destPath = path_1.default.join(inProgressDir, filename);
        fs_1.default.writeFileSync(destPath, content, 'utf-8');
        if (foundZone === 'inbox') {
            fs_1.default.unlinkSync(foundPath);
        }
        ...
        if (Array.isArray(state.open_packets)) {
            state.open_packets = state.open_packets.filter((id) => id !== packetId);
        }
        writeArgosState(state);
...
app.post('/api/tasks/delete', (req, res) => {
    try {
        ...
        const workPacketsDir = path_1.default.join(RUNTIME_DIR, 'work_packets');
        const zones = ['inbox', 'in_progress', 'done'];
        const trashDir = path_1.default.join(workPacketsDir, 'trash');
        ...
        taskIds.forEach((taskId) => {
            ...
            // Search in all zones
            for (const zone of zones) {
                const zoneDir = path_1.default.join(workPacketsDir, zone);
                if (!fs_1.default.existsSync(zoneDir))
                    continue;
                const files = fs_1.default.readdirSync(zoneDir);
                const matchingFile = files.find(f => f.startsWith(`${taskId}__`) || f.includes(`_${taskId}__`) || f === `${taskId}.md`);
                if (matchingFile) {
                    const sourcePath = path_1.default.join(zoneDir, matchingFile);
                    const destPath = path_1.default.join(trashDir, matchingFile);
                    try {
...
app.get('/api/risks', (req, res) => {
    try {
        ...
        if (fs_1.default.existsSync(shadowPath)) {
            const content = fs_1.default.readFileSync(shadowPath, 'utf-8');
            const blocks = content.split('---');
            blocks.forEach(block => {
        ...
        for (const zone of zones) {
            ...
            for (const file of files) {
                ...
                if (content.includes('TYPE: risk')) {
                    ...
                    risks.push({
                        id: packetId,
                        time: packetReportedAt || formatRelativeAge(fs_1.default.statSync(fullPath).mtimeMs),
                        state: packetState,
                        snippet,
        ...
        const collapsedLolaRisks = [];
        const seenLolaSignals = new Set();
        uniqueRisks.forEach((risk) => {
            const riskId = normaliseText(risk.id);
            const isLolaRisk = riskId.startsWith('LOLA-RISK-');
            if (!isLolaRisk) {
                collapsedLolaRisks.push(risk);
                return;
            }
            const normalizedSnippet = normaliseText(risk.snippet).toLowerCase().slice(0, 140);
            const stressValue = normaliseText(String(risk.stress_level ?? ''));
            const signalKey = `${stressValue}|${normalizedSnippet}`;
            if (seenLolaSignals.has(signalKey))
                return;
    ...
    try {
        ...
        [ARGOS_LEGACY_SHADOW_PATH, ARGOS_GLOBAL_SHADOW_PATH, ARGOS_LEGACY_LOG_PATH, ARGOS_GLOBAL_LOG_PATH, ARGOS_LEGACY_GLITCH_PATH, ARGOS_GLOBAL_GLITCH_PATH].forEach(p => {
            if (fs_1.default.existsSync(p)) {
                ...
                blocks.forEach(b => {
                    if (b.includes(query))
                        results.push({ source: path_1.default.basename(p), content: b.trim() });
                });
            }
        });
        getCaptainFeedReadPaths().forEach((feedPath) => {
            if (!fs_1.default.existsSync(feedPath))
                return;
            const lines = fs_1.default.readFileSync(feedPath, 'utf-8').split('\n');
            lines.forEach(l => {
                if (l.includes(query)) {
                    try {
        ...
        [eventsPath, glitchesJsonPath].forEach((jsonlPath) => {
            if (!fs_1.default.existsSync(jsonlPath))
                return;
            const lines = fs_1.default.readFileSync(jsonlPath, 'utf-8').split('\n');
            lines.forEach((line) => {
                ...
                try {
                    ...
                    results.push({
                        source: path_1.default.basename(jsonlPath),
        ...
        const transcriptsPath = path_1.default.join(RUNTIME_DIR, 'events', 'transcripts');
        if (fs_1.default.existsSync(transcriptsPath)) {
...
let fileTreeCache = { generatedAt: 0, tree: [] };
function getFileTree(dir, base = '', depth = 0) {
    let items = [];
    try {
    ...
    return items
        ...
        .map((item) => {
        const fullPath = path_1.default.join(dir, item);
        const relativePath = path_1.default.join(base, item);
        let isDir = false;
        try {
            isDir = fs_1.default.statSync(fullPath).isDirectory();
        }
        catch (error) {
            return null;
        }
...
app.get('/api/files/content', (req, res) => {
    try {
        ...
        if (!relPath)
            return res.status(400).json({ error: 'Falta ruta' });
        // Safety check: ensure path is within RUNTIME_DIR
        const fullPath = path_1.default.resolve(RUNTIME_DIR, relPath);
...
function startAntigravityActivityWatcher() {
    const watchTarget = path_1.default.resolve(path_1.default.join(__dirname, '..', '..')); // En scratch
...
async function callOllama(messages, timeoutMs = DEEPSEEK_TIMEOUT_MS) {
...
app.post('/api/deepseek/analyze-task', async (req, res) => {
    const { packetId, subject, objective } = req.body;
...
// ============================================================
app.get('/api/desktop-import/config', (_req, res) => {
    try {
        const config = loadDesktopSourcesConfig();
...
app.post('/api/desktop-import/config', (req, res) => {
    try {
...
app.get('/api/desktop-import/status', (_req, res) => {
    try {
        const config = loadDesktopSourcesConfig();
        const state = loadDesktopIngestState();
        return res.json({
            ...
            enabledSources: config.sources.filter((s) => s.enabled).map((s) => ({
                ...
                rootExists: fs_1.default.existsSync(s.rootPath)
            })),
...
app.post('/api/desktop-import/run', (req, res) => {
    try {
...
app.get('/api/desktop-import/files', (req, res) => {
    try {
        const sourceId = normaliseText(req.query.source).toLowerCase();
        const kindRaw = normaliseText(req.query.kind).toLowerCase();
        const kind = kindRaw === 'tokens' ? 'tokens' : 'transcripts';
        const limit = Math.max(1, Math.min(5000, Number(req.query.limit || 300)));
        const config = loadDesktopSourcesConfig();
        const source = config.sources.find((s) => s.id === sourceId);
        if (!source)
            return res.status(404).json({ error: `Source desconocida: ${sourceId}` });
        if (!fs_1.default.existsSync(source.rootPath))
            return res.json({ source: source.id, kind, files: [] });
        const files = filesMatchingGlobs(source.rootPath, kind === 'tokens' ? source.tokenGlobs : source.transcriptGlobs)
            ...
            const rel = path_1.default.relative(source.rootPath, abs).replace(/\\/g, '/');
            return {
                relativePath: rel,
                absolutePath: abs,
                size: stat.size,
...
app.get('/api/desktop-import/file-content', (req, res) => {
    try {
        const sourceId = normaliseText(req.query.source).toLowerCase();
        const relPath = normaliseText(req.query.path);
        if (!sourceId || !relPath)
            return res.status(400).json({ error: 'source y path son obligatorios' });
        const config = loadDesktopSourcesConfig();
        const source = config.sources.find((s) => s.id === sourceId);
        if (!source)
            return res.status(404).json({ error: `Source desconocida: ${sourceId}` });
        const absPath = path_1.default.resolve(source.rootPath, relPath);
        const rootResolved = path_1.default.resolve(source.rootPath);
        ...
        if (!fs_1.default.existsSync(absPath))
            return res.status(404).json({ error: `Archivo no encontrado: ${relPath}` });
...
app.get('/api/live', (_req, res) => {
    try {
        ensureLiveLayerBootstrap();
        const agents = ['claude', 'codex', 'gemini', 'openclaw'];
...
app.get('/api/live/:agent', (req, res) => {
    try {
        ensureLiveLayerBootstrap();
...
app.post('/api/live/:agent', (req, res) => {
    try {
        ensureLiveLayerBootstrap();
        const agentId = normalizeLiveAgentId(req.params.agent);
        if (!agentId) {
...
app.post('/api/qwen/run', async (req, res) => {
    const { task, context, systemPrompt, packetId } = req.body;
...
app.post('/api/qwen/execute-packet', async (req, res) => {
    ...
    for (const zone of zones) {
        ...
        try {
            ...
            if (match) {
                packetPath = path_1.default.join(dir, match);
                packetContent = fs_1.default.readFileSync(packetPath, 'utf-8');
                break;
            }
        }
...
function extractAnthropicTokens(body) {
    ...
    if (startMatch)
        input = parseInt(startMatch[1], 10);
    const deltaMatch = body.match(/"message_delta"[\s\S]*?"output_tokens"\s*:\s*(\d+)/);
    if (deltaMatch)
        output = parseInt(deltaMatch[1], 10);
    if (input === 0 && output === 0) {
        const usageMatch = body.match(/"usage"\s*:\s*\{[^}]*"input_tokens"\s*:\s*(\d+)[^}]*"output_tokens"\s*:\s*(\d+)/);
        if (usageMatch) {
            input = parseInt(usageMatch[1], 10);
            output = parseInt(usageMatch[2], 10);
        }
    }
    return input + output;
}
function extractGeminiTokens(body) {
...
app.all('/proxy/:provider/*', async (req, res) => {
    ...
    if (!targetBase)
        return res.status(404).json({ error: `Proveedor desconocido: ${provider}` });
    ...
    try {
        ...
        upstreamRes.headers.forEach((v, k) => { if (k.toLowerCase() !== 'content-encoding')
            res.setHeader(k, v); });
        if (contentType.includes('text/event-stream')) {
            ...
            if (tokens > 0)
                recordTokensInternal(agentName, tokens, `proxy-stream-${provider}`, '', 'work');
        }
        else {
            ...
            if (tokens > 0)
                recordTokensInternal(agentName, tokens, `proxy-${provider}`, '', 'work');
        }
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
...
app.get('/api/sources', (req, res) => {
    try {
        const argosRoot = path_1.default.join(RUNTIME_DIR, '..');
        const detectedSources = [];
        function getRepoInfo(itemPath, name) {
        ...
        // 1. Check root
        if (fs_1.default.existsSync(path_1.default.join(argosRoot, '.git'))) {
            detectedSources.push(getRepoInfo(argosRoot, 'ARGOS (Root)'));
        }
        // 2. Scan subdirectories
        const items = fs_1.default.readdirSync(argosRoot, { withFileTypes: true });
        items.forEach(item => {
            if (item.isDirectory() && item.name !== 'ARGOS_RUNTIME' && !item.name.startsWith('.')) {
                const itemPath = path_1.default.join(argosRoot, item.name);
                if (fs_1.default.existsSync(path_1.default.join(itemPath, '.git'))) {
...
app.post('/api/sources/sync', (req, res) => {
    const { name } = req.body;
    if (!name)
        return res.status(400).json({ error: 'Nombre de fuente requerido' });
    const argosRoot = path_1.default.join(RUNTIME_DIR, '..');
    let targetPath = path_1.default.join(argosRoot, name);
    if (name === 'ARGOS (Root)') {
...
app.listen(PORT, () => {
    ...
    setInterval(() => {
        ...
        try {
            if (!fs_1.default.existsSync(ARGOS_TOKENS_PATH))
                return;
            const lines = fs_1.default.readFileSync(ARGOS_TOKENS_PATH, 'utf-8').replace(/^\uFEFF/, '').split('\n').filter(Boolean);
            const totals = {};
            let grand = 0;
            lines.forEach(l => {
                try {
                    const r = JSON.parse(l);
                    if (r.scope !== 'work' && r.scope !== undefined)
                        return;
        ...
        zones.forEach(zone => {
            ...
            files.forEach(file => {
                ...
                if (!oldStateKey) {
                    ...
                    if (zone === 'done') {
                        const fileAgeMs = Date.now() - fs_1.default.statSync(fullPath).mtimeMs;
                        ...
                        if (missingLanes.length > 0 && fileAgeMs > gracePeriodMs) {
                            ...
                            if (isAntigravityRole(owner))
                                ensureAntigravityProtocolPacketExists();
                        }
                    }
                }
...
function runLolaShadowScanner() {
    ...
    try {
        ...
        indicators.forEach(ind => {
            ...
            if (matches)
                stressLevel += matches.length * ind.weight;
        });
